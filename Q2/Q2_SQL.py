...
class HW2_sql():
    ...
    def part_aii_1(self,connection,path):
        ...
        c = connection.cursor()
        movies_csv_data = open(path, 'r', encoding='utf-8')
        movies_list_read = csv.reader(movies_csv_data)
        movies_list = list()
        for i in movies_list_read:
            list_unit =(i[0],i[1],i[2])
            movies_list.append(list_unit)
        c.executemany("INSERT INTO movies(id, title, score) values (?,?,?);", movies_list)
        connection.commit()
        c.close()
       ######################################################################
    ...
    def part_aii_2(self,connection, path):
        ############### CREATE IMPORT CODE BELOW ############################
        c = connection.cursor()
        movie_cast_csv_data = open(path, 'r', encoding='utf-8')
        movie_cast_read = csv.reader(movie_cast_csv_data)
        movie_cast_list =list()
        for i in movie_cast_read:
            unit_data = (i[0], i[1], i[2], i[3], i[4])
            movie_cast_list.append(unit_data)
        c.executemany("INSERT INTO movie_cast(movie_id, cast_id, cast_name, birthday, popularity) values (?,?,?,?,?);", movie_cast_list)
        connection.commit()
        c.close()
        ######################################################################
    ...
    def part_aiii(self,connection):
        ...
        ############### CREATE IMPORT CODE BELOW ############################
        select ="SELECT cast_id, cast_name, birthday, popularity FROM movie_cast GROUP BY cast_id;"
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = "SELECT printf('%.2f', COUNT(movies.id)*100.00/(SELECT COUNT(movies.id) FROM movies)) " \
                     "FROM movies " \
                     "WHERE movies.score > 50 AND movies.title LIKE '%war%';"
        ######################################################################
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = "SELECT movie_cast.cast_name, COUNT(movie_cast.cast_name) " \
                     "FROM movie_cast " \
                     "GROUP BY movie_cast.cast_name HAVING movie_cast.popularity > 10 " \
                     "ORDER BY COUNT(movie_cast.cast_name) DESC, movie_cast.cast_name " \
                     "LIMIT 5;"
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "SELECT movies.title, printf('%.2f',movies.score), COUNT(movie_cast.cast_id) " \
                     "FROM movies " \
                     "INNER JOIN movie_cast ON movies.id=movie_cast.movie_id " \
                     "GROUP BY movies.id " \
                     "ORDER BY movies.score DESC, COUNT(movie_cast.cast_id), movies.title " \
                     "LIMIT 5"
        ######################################################################
    ...
    def part_f(self,connection):
        ...
        part_f_sql = "SELECT movie_cast.cast_id, movie_cast.cast_name, printf('%.2f',AVG(movies.score)) " \
                     ...
                     "GROUP BY movie_cast.cast_id HAVING movies.score >= 25 AND COUNT(movies.id) > 2 " \
                     "ORDER BY AVG(movies.score) DESC, movie_cast.cast_name " \
                     "LIMIT 10"
        ######################################################################
    ...
    def part_g(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_sql = "CREATE VIEW good_collaboration (cast_member_id1, cast_member_id2, movie_count, average_movie_score) AS " \
                     "SELECT movie_cast.cast_id, movie_cast.cast_id, COUNT(movies.id), AVG(movies.score) " \
                     "FROM movie_cast " \
                     "INNER JOIN movies ON movie_cast.movie_id=movies.id " \
                     "WHERE movies.score >= 40 AND COUNT(movies.id)>3"
        ######################################################################
        return self.execute_query(connection, part_g_sql)
    
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = "SELECT cast_member_id1, printf('%.2f',average_movie_score) " \
                       "FROM good_collaboration " \
                       "ORDER BY good_collaboration.average_movie_score DESC " \
                       "LIMIT 5"
        ######################################################################
    ...
    def part_h(self,connection,path):
        ...
        ############### CREATE IMPORT CODE BELOW ############################
        c = connection.cursor()
        overview_csv_data = open(path, 'r', encoding='utf-8')
        overview_list_read = csv.reader(overview_csv_data)
        overview_list = list()
        for i in overview_list_read:
            list_unit = (i[0], i[1])
            overview_list.append(list_unit)
        c.executemany("INSERT INTO movie_overview(id, overview) values (?,?);", overview_list)
        connection.commit()
        c.close()
        ######################################################################
    ...
    def part_hi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hi_sql = "SELECT COUNT(movie_overview.overview) " \
                      "FROM movie_overview " \
    ...
    def part_hii(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hii_sql = "SELECT COUNT(movie_overview.overview) " \
                       "FROM movie_overview " \
